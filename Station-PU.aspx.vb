...
Partial Class Station
    ...
    Public Function BanList(ByVal Hospcode As String, ByVal Wardcode As String)
        ...
        '刪除兒科病房主治醫師資料
        Dim cmd2 As String = String.Format("SELECT * FROM WHTSCHED WHERE NS = '16' AND trunc(Udate) = to_date('2021/12/07', 'YYYY-MM-DD')")
        Dim DA10 As OleDbDataAdapter = New OleDbDataAdapter(cmd2, Conn1)

        Dim del_SCH_CMD As String = String.Format("DELETE FROM WHTSCHED WHERE NS = '17' AND trunc(Udate) = to_date('2021/12/07', 'YYYY-MM-DD')")
        Dim del_SCH As OleDbCommand = New OleDbCommand(del_SCH_CMD, Conn1)
        Conn1.Open()

        del_SCH.ExecuteNonQuery()

        DA10.Fill(DS, "DEL")
        GridView1.DataSource = DS.Tables("DEL")
        GridView1.DataBind()
        Conn1.Close()
        '建立最後班表輸出時的TABLE
