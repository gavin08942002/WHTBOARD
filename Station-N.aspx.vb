...
Partial Class Station
    ...
    End Sub
    '院區中文名稱
    Public Function Hospcode_to_ID(ByVal Hospcode As String) As String
        Dim Hospname As String = Nothing
    ...
    End Function
    '將LIST 轉換成SQL In 的格式(排除hospcode=1,wardcode=32)
    Public Function List_to_SQLin2(ByVal dept() As String, ByVal Column As String, ByVal RMaster As String) As String
        Dim DeptStr As String = String.Format("AND  {0} in ('{1}')", Column, "沒有值")
        If dept.Count > 0 Then
            DeptStr = String.Format(" AND  {0}  in ('{1}'", Column, dept(0).ToString)  '頭
            For i = 1 To dept.Count - 1                                                                                        '身
    ...
    '取得全院未交班醫師LIST(夜版)
    Public Function ISBARREC_RECVING_ALL(ByVal Hospcode As String, ByVal Wardcode As String, ByVal Btime As Integer, ByVal Etime As Integer, ByVal Now_Time As DateTime) As ArrayList
        'Btime  未交班燈號開始時間
        'Etime  未交班燈號結束時間
        Dim NowTime As Integer = Now_Time.Hour * 100 + Now_Time.Minute
        Dim ConnStr As String = SELECT_ORACLE(Hospcode)
        ...
        Dim cmd_state As String = Nothing
        cmd_state &= " Select i.BEDNS, i.PNO,i.CASENO,i.DTYPE,i.BEDNO,i.SEVERITY,i.TRANSFER,i.RECVING,i.CR,i.CRFOLLOWUP,"
        cmd_state &= " Decode(i.SEVERITY,1,'輕',2,'中',3,'重') DSEVERITY,d.CNAME1 as DeptName,n.NSENG,"
        cmd_state &= " i.Dline,i.recvdate,Dr1.code as TDRcode,Dr1.NAME as TDRName,dr2.code as RDRcode,  Dr2.NAME as RDRName,  Dr3.NAME as CRName,  "
        cmd_state &= " IDP.NAME as Name,IDP.SEX,IDP.BIRTH  "
        cmd_state &= " From ISBARREC i,Dept d,Nstbl n,Dr Dr1,Dr Dr2,Dr Dr3,IDP  "
        ...
        '全院未交班資料
        Dim TDR_LIST As ArrayList = New ArrayList
        TDR_LIST = datarow_to_ArrayList(DT_state.Select, "RECVING")
        '全院輕度未交班資料
        Dim TDR_LIST_L As ArrayList = New ArrayList
        TDR_LIST_L = datarow_to_ArrayList(DT_state.Select("DSEVERITY='輕'"), "RECVING")
        '全院中度未交班資料
        Dim TDR_LIST_M As ArrayList = New ArrayList
        TDR_LIST_M = datarow_to_ArrayList(DT_state.Select("DSEVERITY='中'"), "RECVING")
        '全院重度未交班資料
        Dim TDR_LIST_H As ArrayList = New ArrayList
        TDR_LIST_H = datarow_to_ArrayList(DT_state.Select("DSEVERITY='重'"), "RECVING")


        '護理站未交班資料
        Dim TDR_LIST_NS As ArrayList = New ArrayList
        TDR_LIST_NS = datarow_to_ArrayList(DT_state.Select("BEDNS='" & Wardcode & "'"), "RECVING")
        '全院輕度未交班資料
        Dim TDR_LIST_NS_L As ArrayList = New ArrayList
        TDR_LIST_NS_L = datarow_to_ArrayList(DT_state.Select("DSEVERITY='輕' AND BEDNS='" & Wardcode & "'"), "RECVING")
        '全院中度未交班資料
        Dim TDR_LIST_NS_M As ArrayList = New ArrayList
        TDR_LIST_NS_M = datarow_to_ArrayList(DT_state.Select("DSEVERITY='中' AND BEDNS='" & Wardcode & "'"), "RECVING")
        '全院重度未交班資料
        Dim TDR_LIST_NS_H As ArrayList = New ArrayList
        TDR_LIST_NS_H = datarow_to_ArrayList(DT_state.Select("DSEVERITY='重' AND BEDNS='" & Wardcode & "'"), "RECVING")
        ...
        Return TDR_LIST
    End Function
    Public Function datarow_to_ArrayList(ByVal datarows() As DataRow, ByVal columnNo As String) As ArrayList
        'DataRows 資料列陣列
        'columnNo 要放入陣列的欄位
        Dim List As ArrayList = New ArrayList
        For i = 0 To datarows.GetUpperBound(0)
            Dim temp As String = datarows(i)(columnNo).ToString
            If Not List.Contains(temp) And Not String.IsNullOrEmpty(temp) Then
                List.Add(temp)
            End If
        Next
        Return List
    ...
    Public Sub BanList_NS_N(ByVal Hospcode As String, ByVal Wardcode As String, ByVal Bedcount As Integer, ByVal NSENG As String, ByVal Now_time As DateTime)
        ...
        Info_Show.Columns.Add("DRNAME")
        Info_Show.Columns.Add("GROUP1")
        Info_Show.Columns.Add("PHONE")
        ...
        Info_Show2.Columns.Add("DRNAME")
        Info_Show2.Columns.Add("GROUP1")
        Info_Show2.Columns.Add("PHONE")
        Info_Show2.Columns.Add("MARSTER")
        Info_Show2.Columns.Add("hold1")
        Info_Show2.Columns.Add("hosd2")
        ...
        For i = 0 To MRD_TB.Rows.Count - 1
            Dim info_show_rows As DataRow = Info_Show.NewRow
            ' info_show_rows("DEPT") = MRD_TB.Rows(i)("DEPT").ToString
            info_show_rows("CNAME1") = MRD_TB.Rows(i)("CNAME1").ToString
            'info_show_rows("DRNAME") = MRD_TB.Rows(i)("NAME").ToString
            info_show_rows("DRNAME") = MRD_TB.Rows(i)("DRNAME").ToString
            info_show_rows("GROUP1") = MRD_TB.Rows(i)("GROUP1").ToString
            info_show_rows("PHONE") = MRD_TB.Rows(i)("PHONE").ToString
            Info_Show.Rows.Add(info_show_rows)
        Next


        '***************************************台北->大內科(IM)
        '***************************************台北->大外科(SG)
        If Hospcode = "1" Or Hospcode = "2" Then
            Dim ADD_TB As DataTable = Get_add(Hospcode, Wardcode, Now_time) '外科新增其他科值班醫師資料

            For j = 0 To ADD_TB.Rows.Count - 1
                Dim add_info_show_rows As DataRow = Info_Show2.NewRow
                add_info_show_rows("CNAME1") = ADD_TB.Rows(j)("master").ToString & ""
                add_info_show_rows("DRNAME") = ADD_TB.Rows(j)("NAME").ToString
                add_info_show_rows("GROUP1") = ADD_TB.Rows(j)("GROUP1").ToString
                ' add_info_show_rows("PHONE") = ADD_TB.Rows(j)("PHONE").ToString
                Info_Show2.Rows.Add(add_info_show_rows)
            Next
        End If
        '***************************************台北->婦科(GO)
        If (Hospcode = "1") And DUTY_LIST_Str.Contains("GO") Then
            Dim Shift As String = "('W')" ' 顯示班表病房區
            ' If Wardcode = "32" Then '顯示班表DR區
            '   Shift = "('DR')"
            'End If
            Dim ADD_TB As DataTable = Get_GO_add(Hospcode, Wardcode, Shift, Now_time)

            For j = 0 To ADD_TB.Rows.Count - 1
                Dim add_info_show_rows As DataRow = Info_Show2.NewRow
                'add_info_show_rows("CNAME1") = ADD_TB.Rows(j)("CNAME1").ToString & "G"
                add_info_show_rows("CNAME1") = "婦產"
                add_info_show_rows("DRNAME") = ADD_TB.Rows(j)("NAME").ToString
                add_info_show_rows("GROUP1") = ADD_TB.Rows(j)("GROUP1").ToString
                ' add_info_show_rows("PHONE") = ADD_TB.Rows(j)("PHONE").ToString
                Info_Show2.Rows.Add(add_info_show_rows)
        ...
        '***************************************淡水->婦科(GO)
        If (Hospcode = "2") And DUTY_LIST_Str.Contains("GO") Then
            Dim Shift As String = "('T')" ' 顯示班表病房區
            ' If Wardcode = "32" Then '顯示班表DR區
            '   Shift = "('DR')"
            'End If
            Dim ADD_TB As DataTable = Get_GO_add(Hospcode, Wardcode, Shift, Now_time)
        ...
        '***************************************淡水->兒科(PD)
        '
        Dim ADD_OT_TB As DataTable = Get_OT_add(Hospcode, Wardcode, Now_time)
        For j = 0 To ADD_OT_TB.Rows.Count - 1
            Dim add_info_show_rows As DataRow = Info_Show2.NewRow
            'add_info_show_rows("CNAME1") = ADD_TB.Rows(j)("CNAME1").ToString & "G"
            add_info_show_rows("CNAME1") = ADD_OT_TB.Rows(j)("CNAME1").ToString
            add_info_show_rows("DRNAME") = ADD_OT_TB.Rows(j)("NAME").ToString
            add_info_show_rows("GROUP1") = ADD_OT_TB.Rows(j)("GROUP1").ToString
            ' add_info_show_rows("PHONE") = ADD_TB.Rows(j)("PHONE").ToString
            Info_Show2.Rows.Add(add_info_show_rows)
        Next
    ...
    '====ON CALL醫師資料SQL
    Public Function BanList_NS_N_SQL(ByVal Hospcode As String, ByVal Wardcode As String, ByVal Bedcount As Integer, ByVal NSENG As String, ByVal Now_Time As DateTime) As DataTable
        ...
        Dim ConnStr As String = SELECT_ORACLE(Hospcode)
        Dim TpConnStr As String = SELECT_ORACLE("1")
        Dim Conn As OleDbConnection = New OleDbConnection
        Dim TpConn As OleDbConnection = New OleDbConnection
        Conn.ConnectionString = ConnStr
        ...
        Dim Master As String = Get_Master(Hospcode, Wardcode)
        '護理站的值班醫師科別列表字串
        Dim DUTY_LIST_Str As String = Nothing
        DUTY_LIST_Str = GET_ONCALL(Hospcode, Wardcode)

        'GridView1.DataSource = DS.Tables("DUTY_LIST")
        'GridView1.DataBind()

        Dim DUTY_LIST As String = Nothing

        '將List資料轉為 SQL IN的格式
        If Not String.IsNullOrEmpty(DUTY_LIST_Str) Then
            DUTY_LIST = List_to_SQLin(DUTY_LIST_Str.Split(","), " INPDRSCH.DEPT  ")
        Else
            DUTY_LIST = List_to_SQLin({}, " INPDRSCH.DEPT ")
        End If

        Dim cmd As String = Nothing

        cmd &= " select  INPDRSCH.DEPT, DECODE(DR.CLASS,'XX',DECODE( SUBSTR(DR.CODE,1,1),'Z','INT','PGY') ,CLASS) as classes "
        cmd &= " , DEPT.CNAME1 , INPDRSCH.DRCODE, DR.NAME DRNAME, PHSDB.GROUP1 , PHSDB.CODE as PHONE , "
        cmd &= " INPDRSCH.SCHDATE ,decode( INPDRSCH.SHIFT, 'D' , '白天' , 'N' , '晚上') shift, INPDRSCH.MASTER , "
        cmd &= " INPDRSCH.HOSPID ,"
        cmd &= " INPDRSCH.URGENT ,  INPDRSCH.VS,decode( INPDRSCH.VS , 'Y' , '主治醫師' , 'N' , '住院醫師'  ) VS "
        cmd &= " , INPDRSCH.order_num"
        cmd &= " from   INPDRSCH,DR, DEPT , PHSDB "
        cmd &= " where  INPDRSCH.DRCODE = DR.CODE and DEPT.CODE = INPDRSCH.DEPT "
        cmd &= " and PHSDB.EMPNO  = DR.CODE and PHSDB.EMPNO  = INPDRSCH.DRCODE(+)"
        ...
        cmd &= DUTY_LIST
        'cmd &= " and inpdrsch.dept in ('10','55','50','40')" '???????????????????????????????????
        cmd &= " order by INPDRSCH.DEPT,  DR.NAME "


        Dim DA As OleDbDataAdapter = New OleDbDataAdapter(cmd, TpConn) '???????????????????????????????????????????????????????
        Dim Dept_List_DT As DataTable = New DataTable
        DA.Fill(Dept_List_DT)
        'GridView3.DataSource = Dept_List_DT
        'GridView3.DataBind()
        If (Conn.State = ConnectionState.Open) Then
            Conn.Close()
            Conn.Dispose()
    ...
    End Function
    '====四大科(SG,IM,PD,GO)值班醫師SQL
    Public Function Get_add(ByVal Hospcode As String, ByVal Wardcode As String, ByVal Now_Time As DateTime) As DataTable
        '現在時間
        Dim Nowdate As String = Now_Time.ToString("yyyy/MM/dd")
        If Now_Time.Hour < 8 Then
            Nowdate = Now_Time.AddDays(-1).ToString("yyyy/MM/dd")
        ...
        '護理站的值班醫師科別列表字串
        Dim DUTY_LIST_Str As String = Nothing
        DUTY_LIST_Str = GET_DUTY_LIST(Hospcode, Wardcode)
        '台北SI  duty選IM時,抓MI_B的mstr  
        If Hospcode = "1" And Wardcode = "34" Then
            DUTY_LIST_Str = Get_Master(Hospcode, "26")
            Wardcode = "26"
        End If
        ...
        '將List資料轉為 SQL IN的格式
        If Not String.IsNullOrEmpty(DUTY_LIST_Str) Then
            DUTY_LIST = List_to_SQLin2(DUTY_LIST_Str.Split(","), " A.MASTER  ", "GO")
        Else
            DUTY_LIST = List_to_SQLin({}, " A.MASTER  ")
        End If

        Dim cmd As String = Nothing
        cmd &= " select distinct A.hospid, B.dept as deptcode  ,E.group1, C.cname1 , A.drcode , B.name , "
        cmd &= " DECODE(B.CLASS,'XX ',DECODE(SUBSTR(B.CODE,1,1),'Z','INT','PGY'),B.CLASS) as CLASS   "
        cmd &= " , decode(A.master,'IM','內科','SG','外科','GO','婦科','PD','兒科',A.master) master, A.shift , TO_CHAR(A.SCHDATE,'yyyy/mm/dd') as SCHDATE   "
        cmd &= "  ,   to_number(  decode(   DECODE(B.CLASS,'XX ',DECODE(SUBSTR(B.CODE,1,1),'Z','INT','PGY'),B.CLASS)  ,  'INT', '1' ,  'PGY' , '2' ,  'R1 ', '3' , 'R2 ' , '4' , 'R3 ' , '5' , 'R4 ' , '6' , 'R5 ' , '7'  , 'R6 ' , '8' ,  DECODE( SUBSTR(class  ,1,1)    ,'B' , class ) , '9' ,   DECODE( SUBSTR(class  ,1,1)      , 'A'  , class ), '10' ,  DECODE( SUBSTR(class  ,1,1)    ,'S' , class )  ,'11'  )   )  as classorder"
        ...
        cmd &= String.Format(" and D.location = '{0}'  ", Wardcode)
        ...
        cmd &= " order by  master, classorder"
        Dim DA As OleDbDataAdapter = New OleDbDataAdapter(cmd, TpConn)
        DA.Fill(DS, "ADD")
    ...
    End Function
    '====其他科值班醫師SQL
    Public Function Get_OT_add(ByVal Hospcode As String, ByVal Wardcode As String, ByVal Now_Time As DateTime) As DataTable
        '現在時間
        Dim Nowdate As String = Now_Time.ToString("yyyy/MM/dd")
        If Now_Time.Hour < 8 Then
            Nowdate = Now_Time.AddDays(-1).ToString("yyyy/MM/dd")
        ...
        End If
        Dim DUTY_LIST As String = Nothing
        Dim DUTY_LIST2 As String = Nothing
        '將List資料轉為 SQL IN的格式
        If Not String.IsNullOrEmpty(DUTY_LIST_Str) Then
            DUTY_LIST = List_to_SQLin2(DUTY_LIST_Str.Split(","), " D.dept  ", "GO")
        Else
            DUTY_LIST = List_to_SQLin({}, " D.dept  ")
        End If
        If Not String.IsNullOrEmpty(DUTY_LIST_Str) Then
            DUTY_LIST2 = List_to_SQLin2(DUTY_LIST_Str.Split(","), " A.dept  ", "GO")
        Else
            DUTY_LIST2 = List_to_SQLin({}, " A.dept  ")
        End If

        Dim cmd As String = Nothing
        cmd &= " select distinct A.hospid, B.dept as deptcode  ,E.group1, C.cname1 , A.drcode , B.name ,  "
        cmd &= " DECODE(B.CLASS,'XX ',DECODE(SUBSTR(B.CODE,1,1),'Z','INT','PGY'),B.CLASS) as CLASS   "
        cmd &= " , A.master, A.shift , TO_CHAR(A.SCHDATE,'yyyy/mm/dd') as SCHDATE "
        cmd &= "  ,   to_number(  decode(   DECODE(B.CLASS,'XX ',DECODE(SUBSTR(B.CODE,1,1),'Z','INT','PGY'),B.CLASS)  ,  'INT', '1' ,  'PGY' , '2' ,  'R1 ', '3' , 'R2 ' , '4' , 'R3 ' , '5' , 'R4 ' , '6' , 'R5 ' , '7'  , 'R6 ' , '8' ,  DECODE( SUBSTR(class  ,1,1)    ,'B' , class ) , '9' ,   DECODE( SUBSTR(class  ,1,1)      , 'A'  , class ), '10' ,  DECODE( SUBSTR(class  ,1,1)    ,'S' , class )  ,'11'  )   )  as classorder "
        cmd &= " from  inpdrsch A , dr_his B , dept C , drschloc D , PHSDB  E"
        cmd &= " where A.drcode = B.code and C.code = B.dept "
        cmd &= " and  A.shift = D.shift  and A.sch_type = 'S'  and E.EMPNO = A.drcode "
        cmd &= String.Format(" and A.HospID = '{0}' ", Hospcode)
        ' cmd &= " and D.dept in ('52','53','54','55','SG','IM','PD','GO')"
        'cmd &= " and A.dept in ('52','53','54','55','SG','IM','PD','GO')"
        cmd &= DUTY_LIST
        cmd &= DUTY_LIST2
        'cmd &= String.Format(" and D.dept in {0}")
        cmd &= String.Format(" and trunc ( to_date( '{0}' , 'yyyy/mm/dd' )) >= D.bdate   ", Nowdate)
        cmd &= String.Format("and  trunc ( to_date( '{0}' , 'yyyy/mm/dd' )) <= D.Edate  ", Nowdate)
        cmd &= String.Format(" and D.location = '{0}'  ", Wardcode)
        cmd &= String.Format(" and A.schdate = to_date( '{0}' , 'yyyy/mm/dd' )   ", Nowdate)
    ...
    End Function
    '====婦科值班醫師SQL
    Public Function Get_GO_add(ByVal Hospcode As String, ByVal Wardcode As String, ByVal Shift As String, ByVal Now_Time As DateTime) As DataTable
        '搜尋條件==婦產科Master= GO, Shift = W
        Dim DS As DataSet = New DataSet
        '現在時間
        Dim Nowdate As String = Now_Time.ToString("yyyy/MM/dd")
        If Now_Time.Hour < 8 Then
        ...
        Dim cmd As String = Nothing
        cmd &= " select distinct G.CNAME1,  D.DRCODE, E.NAME,  DECODE(E.CLASS,'XX ',DECODE(SUBSTR(E.CODE,1,1),'Z','INT','PGY'),CLASS) as class, F.Group1  , D.MASTER ,D.SHIFT ,C.nseng  ,TO_CHAR(D.SCHDATE,'yyyy/mm/dd') as SCHDATE , NVL(D.HOSPID,'XXX') as HOSPID "
        cmd &= " ,to_number( decode(DECODE(E.CLASS,'XX ',DECODE(SUBSTR(E.CODE,1,1),'Z','INT','PGY'),E.CLASS)  ,'INT', '1' , 'PGY' ,'2' , 'R1 ', '3' ,'R2 ' ,'4' ,'R3 ' ,'5' ,'R4 ' , '6' , 'R5 ' , '7'  , 'R6 ' , '8' ,  DECODE( SUBSTR(class  ,1,1),'B' , class ) , '9' , DECODE( SUBSTR(class  ,1,1)  , 'A' , class ), '10' ,  DECODE( SUBSTR(class  ,1,1)    ,'S' , class )  ,'11'  )   )  as classorder "
        cmd &= " from   INPDRSCH D,DR_his E,  "
        cmd &= " (select shift, listagg( nseng, '+') within group ( order by ns )  nseng  "
        cmd &= " from    (  select shift ,  decode ( nseng , '' ,  location  , nseng , nseng )    nseng  ,  dept , ns  "
        cmd &= " from   (  select A.shift ,  B.nseng    , A.dept , A.location ,B.ns  "
        cmd &= " from drschloc  A,  "
        cmd &= " ( Select Ns,NSCHINES,NSENG From NsTbl Where CSDP is not null  "
        cmd &= " Union (Select Ns,NSCHINES,NSENG From NsTbl1 Where CSDP is not null ) order by 2) B   "
        cmd &= " where A.location= B.ns (+)     and dept = 'GO'  "
        cmd &= String.Format(" and trunc ( to_date('{0}', 'yyyy/mm/dd')   ) >= A.bdate  ", Nowdate)
        cmd &= String.Format(" and  trunc ( to_date('{0}', 'yyyy/mm/dd')   ) <= A.Edate  ", Nowdate)
        cmd &= " order by B.ns  ) ) "
        cmd &= " group by shift ) C,   "
        cmd &= " PHSDB F, "
        cmd &= " DEPT G "
        cmd &= " where  D.DRCODE = E.CODE "
        cmd &= " and  G.code  = D.DEPT  "
        cmd &= " and  F.EMPNO  = D.DRCODE  "
        cmd &= " and  C.SHIFT  = D.SHIFT  "
        cmd &= String.Format(" and  D.SCHDATE  = to_date('{0}', 'yyyy/mm/dd')  ", Nowdate)
        cmd &= "and D.SCH_TYPE = 'S' "
        cmd &= " and D.MASTER = 'GO'  "
        cmd &= String.Format(" and 	e.bdate <= to_date('{0}', 'yyyy/mm/dd') and e.edate >= to_date('{0}', 'yyyy/mm/dd')", Nowdate)
        cmd &= String.Format(" and D.HOSPID = '{0}' ", Hospcode)
        cmd &= String.Format(" and D.SHIFT in {0}", Shift)
        cmd &= " order by  classorder "

        Dim DA As OleDbDataAdapter = New OleDbDataAdapter(cmd, TpConn)
        DA.Fill(DS, "DR_ADD")
        'GridView1.DataSource = DS.Tables("DR_ADD")
        'GridView1.DataBind()
        'Response.Write("TEST")
        Return DS.Tables("DR_ADD")
    End Function

    '====顯示值班醫師資料(單一區)(未使用)
    Public Sub show_doctor_NS_N(ByVal doctor_info_sort As DataTable, ByVal Doctor_Limite As Integer)
        'Doctor_Limite 欄位數
        ...
        For w = 0 To (doctor_info_sort.Rows.Count - 1)
            ...
            Dim DEPT As String = doctor_info_sort.Rows(w)("DEPT").ToString
            'Dim CNAME1 As String = Adjust_Font_size(doctor_info_sort.Rows(w)("CNAME1").ToString, 4, 30)
            Dim CNAME1 As String = doctor_info_sort.Rows(w)("CNAME1").ToString
            Dim DRNAME As String = Adjust_Font_size(doctor_info_sort.Rows(w)("DRNAME").ToString, 4, 30)
            Dim GROUP1 As String = doctor_info_sort.Rows(w)("GROUP1").ToString
            ...
            Doctor_Info_show &= String.Format("          <div style=""height:58px; width:125px; float:left; font-size:40px; line-height:50px; text-align:center; background-color:#F0A75F;"">{0}</div>", CNAME1)

            Doctor_Info_show &= String.Format("    </div>")

            Doctor_Info_show &= String.Format("    <div style=""height:60px;width:125px;float:left;"">")
            Doctor_Info_show &= String.Format("          <div style=""height:40px; width:125px; float:left; font-size:40px; line-height:34px; text-align:center; background-color:#E0FFFF;"">{0}</div>", DRNAME)
            Doctor_Info_show &= String.Format("          <div style=""height:18px; width:125px; float:left; font-size:30px; line-height:16px; text-align:center; background-color:#E0FFFF; font-family:Arial;"">{0}</div>", GROUP1)
            Doctor_Info_show &= String.Format("    </div>")

            Doctor_Info_show &= String.Format("    <div style=""height:60px;width:125px;float:left;"">")
            Doctor_Info_show &= String.Format("          <div style=""height:40px; width:125px; float:left; font-size:40px; line-height:34px; text-align:center; background-color:#BAFDFD;"">{0}</div>", "")
            Doctor_Info_show &= String.Format("          <div style=""height:18px; width:125px; float:left; font-size:30px; line-height:16px; text-align:center; background-color:#BAFDFD; font-family:Arial;"">{0}</div>", "")
            Doctor_Info_show &= String.Format("    </div>")
    ...
    End Sub
    '====顯示值班醫師資料(分一、二區)
    Public Sub show_doctor_NS_N_1(ByVal doctor_info_sort As DataTable, ByVal Doctor_Limite As Integer, ByVal ColNo As String)
        'Doctor_Limite 欄位數
        'ColNo  顯示在第一區或第二區
        Dim Doctor_Info_show1 As String = Nothing 'Doctor1醫生資料輸出第一列
        Dim Doctor_Row_count As Integer = 0
        ...
        For w = 0 To (doctor_info_sort.Rows.Count - 1)
            ...
            If (Doctor_Row_count <= Doctor_Limite) Then
                Doctor_Info_show1 &= Doctor_Info_show
            End If
    ...
    Public Function OnDutyDoctor(ByVal Hospcode As String, ByVal Wardcode As String, ByVal Now_Time As DateTime)
        ...
        '新增排除drschloc 歷史資料
        cmd2 &= "and A.shift not in(   select  shift from drschloc where location = '總值'   and dept in ('PO') and bdate <= ( select sysdate from dual  ) and edate >=  ( select sysdate from dual  )     ) " '排除兒科總值和值班醫師重複的情形
        ...
        cmd4 &= "and A.shift not in(   select  shift from drschloc where location = '總值'   and dept in ('PO') and bdate <= ( select sysdate from dual  ) and edate >=  ( select sysdate from dual  )     ) " '排除兒科總值和值班醫師重複的情形
    ...
    Protected Sub Page_Load(sender As Object, e As EventArgs) Handles Me.Load
        Dim HospCode As String = Request.QueryString("Hospcode")
        Dim WardCode As String = Request.QueryString("WardCode")
        Dim HospID As String = Hospcode_to_ID(HospCode) '院區中文名稱
        ...
        Nurse_count = Nurse_List(HospCode, WardCode, 9, 20, NsSort, Dancag, Now_Time) '列出護士班表資料
        BanList_NS_N(HospCode, WardCode, 14, NSENG, Now_Time) '醫師值班班表
